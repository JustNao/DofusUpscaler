...
class D2PReader:
    ...
    def __init__(self, stream, autoload=True):
        ...
        while i < self._number_properties:
            try:
                property_type = (D2P_file_binary.read_string()).decode('ISO-8859-1')
                property_value = (D2P_file_binary.read_string()).decode('ISO-8859-1')
            except ValueError:
                property_type = (D2P_file_binary.read_string()).decode("ISO-8859-1")
                property_value = (D2P_file_binary.read_string()).decode("ISO-8859-1")
...
class D2PBuilder:
    ...
    def build(self):
        ...
        for ppty_type, ppty_value in self._template._properties.items():
            D2P_file_build_binary.write_string(ppty_type.encode("ISO-8859-1"))
            D2P_file_build_binary.write_string(ppty_value.encode("ISO-8859-1"))
